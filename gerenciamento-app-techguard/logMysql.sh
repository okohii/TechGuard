...
# Inicia a gravação das informações no arquivo de log.
{
  ...
  echo "Logs do Docker (TechGuardDB):"
  sudo docker logs TechGuardDB 2>&1  # Redireciona a saída e erros do Docker para o log.
...
aws s3 cp "$LOG_FILE" s3://$S3_BUCKET/logs/logsDB/ >> "$LOG_FILE" 2>&1
